...
public class HatchingGroundTests {
    ...
    public void testAddElementsToAllAdjacentEggsEffectOneColor() {
        ...
        ElementSpace secondCardSecondMockElementSpace = EasyMock
                .partialMockBuilder(ElementSpace.class).addMockedMethod("addElements").createMock();
        secondCardSecondMockElementSpace.color = ElementColor.BLUE;

        adjacentCard.elementSpaces =
                new ElementSpace[] {firstMockElementSpace, secondMockElementSpace};
        secondAdjacentCard.elementSpaces =
                new ElementSpace[] {secondCardFirstElementSpace, secondCardSecondMockElementSpace};
        ...
        EasyMock.expect(mockElementSpaceLogic.getPlayableSpaces(Arrays.asList(elementColorsToAdd),
                adjacentCard))
                .andReturn(Arrays.asList(firstMockElementSpace, secondMockElementSpace));
        EasyMock.expect(mockElementSpaceLogic.getPlayableSpaces(Arrays.asList(elementColorsToAdd),
                secondAdjacentCard))
                .andReturn(Arrays.asList(secondCardFirstElementSpace,
                        secondCardSecondMockElementSpace));

        EasyMock.expect(mockElementSpaceLogic.getValidAdditions(firstMockElementSpace))
                .andReturn(Arrays.asList(ElementColor.BLUE));
        EasyMock.expect(mockElementSpaceLogic.getValidAdditions(secondMockElementSpace))
                .andReturn(Arrays.asList(ElementColor.BLUE));

        EasyMock.expect(mockElementSpaceLogic.getValidAdditions(secondCardFirstElementSpace))
                .andReturn(Arrays.asList(ElementColor.BLUE));
        EasyMock.expect(mockElementSpaceLogic.getValidAdditions(secondCardSecondMockElementSpace))
                .andReturn(Arrays.asList(ElementColor.BLUE));
        ...
        EasyMock.replay(mockHatchingGround, mockElementSpaceLogic, firstMockElementSpace,
                secondMockElementSpace, secondCardFirstElementSpace,
                secondCardSecondMockElementSpace, mockElement, mockElementBag);

        addElementsEffect.apply();

        EasyMock.verify(mockHatchingGround, mockElementSpaceLogic, firstMockElementSpace,
                secondMockElementSpace, secondCardFirstElementSpace,
                secondCardSecondMockElementSpace, mockElement, mockElementBag);
    }
