...
public class DragonPhaseTests {
    ...
    public void init() {
        ...
        this.spaces[0].elements =
                Arrays.asList(ElementColor.BLUE, ElementColor.RED);
        this.eggs = Arrays.asList(card);

        EasyMock.expect(player.getHandlerCount()).andReturn(1).anyTimes();
        EasyMock.expect(player.getHandlers()).andReturn(Arrays.asList(handler))
                .anyTimes();
    }
    ...
    public void testSetupWithPurple() {
        ...
        Phase phase =
                new DragonPhase(players, null, bag, hatchingGround, null, null);
        phase.setup();
    ...
    public void testSetupWithOrange() {
        ...
        this.card.elementSpaces = this.spaces;
        this.spaces[0].elements =
                Arrays.asList(ElementColor.RED, ElementColor.YELLOW);
        EasyMock.expect(hatchingGround.pullAndReplaceCompleteEggs())
                .andReturn(eggs);
        bag.putElement(ElementColor.RED);
    ...
    public void testMultiSetup() {
        ...
        this.card.elementSpaces = this.spaces;
        this.spaces[0].elements =
                Arrays.asList(ElementColor.YELLOW, ElementColor.RED);
        Card card2 = new Card();
        ElementSpace[] spaces2 = {new ElementSpace(ElementColor.PURPLE),
                new ElementSpace(ElementColor.RED)};
    ...
    public void testOneUnhatchedEgg() {
        ...
        EasyMock.expect(card.domesticEffects[0].on(player))
                .andReturn(card.domesticEffects[0]);
        EasyMock.expect(card.domesticEffects[0].on(bag))
                .andReturn(card.domesticEffects[0]);
        ...
        EasyMock.replay(hatchingGround, bag, player, card.domesticEffects[0],
                handler);

        Phase phase =
                new DragonPhase(players, gui, bag, hatchingGround, null, null);
        phase.setup();
        phase.turn(player);
        EasyMock.verify(hatchingGround, bag, player, card.domesticEffects[0],
                handler);
    }
    ...
    public void testTwoUnhatchedEggsSamePlayer() {
        ...
        EasyMock.expect(card.domesticEffects[0].on(player))
                .andReturn(card.domesticEffects[0]).times(2);
        EasyMock.expect(card.domesticEffects[0].on(bag))
                .andReturn(card.domesticEffects[0]).times(2);
        card.domesticEffects[0].apply();
        EasyMock.expectLastCall().times(2);
    ...
    public void testOneUnhatchedEggWithTwoEffects() {
        ...
        EasyMock.expect(card.domesticEffects[0].on(player))
                .andReturn(card.domesticEffects[0]);
        EasyMock.expect(card.domesticEffects[1].on(player))
                .andReturn(card.domesticEffects[1]);
        EasyMock.expect(card.domesticEffects[0].on(bag))
                .andReturn(card.domesticEffects[0]);
        EasyMock.expect(card.domesticEffects[1].on(bag))
                .andReturn(card.domesticEffects[1]);
        card.domesticEffects[0].apply();
        ...
        EasyMock.replay(hatchingGround, bag, player, card.domesticEffects[0],
                handler, card.domesticEffects[1]);
    ...
    public void testTwoUnhatchedEggsDifferentPlayers() {
        ...
        EasyMock.expect(card2.domesticEffects[0].on(player2))
                .andReturn(card2.domesticEffects[0]);
        EasyMock.expect(card2.domesticEffects[0].on(bag))
                .andReturn(card2.domesticEffects[0]);
        ...
        EasyMock.replay(hatchingGround, bag, player, card.domesticEffects[0],
                handler, player2, card2.domesticEffects[0], handler2);
    ...
    public void testTwoUncompletedEgg() {
        ...
        EasyMock.expectLastCall().times(2);
        EasyMock.expect(hatchingGround.pullAndReplaceCompleteEggs())
                .andReturn(Arrays.asList(card, card2));
        handler.moveToState(HandlerState.INCUBATION);
        EasyMock.expectLastCall().times(2);
        EasyMock.expect(player.getPlayerId()).andReturn(playerId).anyTimes();
        gui.notifyAction(playerId, message);
    ...
    public void testTwoUncompletedDifferentPlayerEgg() {
        ...
        EasyMock.replay(hatchingGround, bag, player, card.domesticEffects[0],
                handler, player2, card2.domesticEffects[0], handler2, gui);
