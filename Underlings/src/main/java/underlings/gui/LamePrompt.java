...
public class LamePrompt implements PromptHandler {
    ...
    public <T extends Choice> T promptChoice(String prompt, List<T> choices,
            int playerId) {
        int index = this.displayOptions(choices.toArray(), "Player " + playerId,
                prompt);
        return choices.get(index);
    }

    private int displayOptions(Object[] options, String title, String message) {
        int value = JOptionPane.showOptionDialog(null, message, title,
    ...
    public boolean promptDecision(String question, int playerId) {
        int option = JOptionPane.showConfirmDialog(null, question,
                "Player " + playerId, JOptionPane.YES_NO_OPTION);
        return option == JOptionPane.YES_OPTION;
    }

    @Override
    public void displayMessage(String message, int playerId, int icon) {
        JOptionPane.showMessageDialog(null, message, "Player " + playerId,
                icon);
    }
