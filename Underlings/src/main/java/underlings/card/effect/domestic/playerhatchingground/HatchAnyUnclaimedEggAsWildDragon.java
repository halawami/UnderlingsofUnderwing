...
public class HatchAnyUnclaimedEggAsWildDragon extends PlayerHatchingGroundEffect {
    ...
    protected void apply(HatchingGround hatchingGround, EggHatchingUtilities hatchingLogic, Player currentPlayer, Gui gui) {
        YesNoChoice choice = gui.promptChoice(LocaleUtilities.get("prompt_choice_hatch_wildly"), YesNoChoice.getChoices(),
                currentPlayer.getId());
        if (choice == YesNoChoice.YES) {
            ...
            if (unclaimedEggs.size() == 0) {
                ...
                Card toHatch = gui
                        .promptChoice(LocaleUtilities.get("prompt_card_hatch_wildly"), unclaimedEggs, currentPlayer.id);
